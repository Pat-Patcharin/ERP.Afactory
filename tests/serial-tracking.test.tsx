...
describe("Serial Tracking — drawer and detail", () => {
  ...
  it("opens the record when a row is clicked", async () => {
    ...
    /* The row key is product|serial, not the serial alone — a serial number is
       only unique within its product. */
    expect(routerPush).toHaveBeenCalledWith(
      `/m/serial-tracking/${encodeURIComponent("XRY-GT1|GT1-TH-000128")}`,
    );
  });
...
describe("Serial Tracking — navigation", () => {
  ...
  it("ซ่อนจากเมนู Inventory แต่โมดูลยังอยู่ครบ", () => {
    ...
    expect(NAV.flatMap((g) => g.items).some((i) => i.label === "Serial Tracking")).toBe(
      false,
    );
    /* Still registered, so the route still resolves for anyone who has the
       URL — hidden is not deleted. */
    expect(getSchemas("serial-tracking")).toBeTruthy();
  });
