...
export interface PriceListItem {
  ...
  /* ------------------------------------------------------------------
     โปรฯ ที่ติดมากับ SKU จากแคตตาล็อกผู้ผลิต — ข้อมูลอ้างอิงเท่านั้น

     สี่ช่องนี้ **ไม่ได้ต่อเข้าเครื่องคิดราคา** ไม่มีเอกสารขายใบไหน
     เคยได้ของแถมจากมัน 234 แถวจาก 807 กรอก min/free ครบทั้งคู่ แต่ทั้ง
     repo อ้างถึงสองช่องนั้นแค่ที่นี่กับช่องแสดงผลในแท็บ Promotion ของ
     schemas/price-list-master.tsx เท่านั้น — ไม่มีในเทสต์ ไม่มีใครคูณ

     `hasPromo` ใน price-master.ts ก็ไม่ได้อ่านสองช่องนี้ มัน derive จาก
     promo_catalog || promo_legacy ซึ่งเป็นข้อความ แล้วป้อนแค่แท็บกับ
     filter chip

     **ห้ามต่อเข้าเครื่องคิดราคาโดยไม่ผ่านการตัดสินใจทางธุรกิจ** — การต่อสาย
     คือการเปิดโปร 234 ตัวพร้อมกันในวันเดียว ไม่มีคนอนุมัติ ไม่มีงบ ไม่มีวัน
     หมดอายุ และไม่มีใครรู้ว่ามีอยู่ ไม่ใช่การรักษาพฤติกรรมเดิม เพราะไม่เคยมี
     พฤติกรรมเดิมให้รักษา

     โมดูล Promotion ที่กำลังทำไม่อ่านสี่ช่องนี้ โปรที่ใช้จริงคือแคมเปญที่คน
     ตั้งเอง มีช่วงเวลา มีงบ มีคนอนุมัติ — คนละธรรมชาติกัน

     ดู BACKLOG ข้อ PM-1 ก่อนคิดจะเปลี่ยนเรื่องนี้
     ------------------------------------------------------------------ */
